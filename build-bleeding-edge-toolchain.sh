...
while [ ${#} -gt 0 ]; do
	case ${1} in
		...
		--quiet)
			...
			;;
		*)
			echo "Usage: $0 [--enable-win32] [--enable-win64] [--enable-pi] [--keep-build-folders] [--resume] [--skip-documentation] [--skip-nano-libraries] [--quiet]" >&2
			exit 1
	esac
	shift
done
...
if [ ! -f "${packageArchiveNative}" ]; then
